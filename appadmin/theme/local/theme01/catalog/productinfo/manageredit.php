...
$(document).ready(function(){
	...
	$(".unit table tbody tr td .fa-trash-o").click(function(){
...
function getCertData(product_id,i){												
	$.ajax({
		url:'<?= CUrl::getUrl("catalog/productinfo/getproductcert",['product_id'=>$product_id]); ?>',
		async:false,
		timeout: 80000,
		dataType: 'json', 
		type:'get',
		...
		success:function(data, textStatus){
			if(data.return_status == "success"){
				jQuery(".cert_tree").html(data.menu);
				// $.fn.zTree.init($(".category_tree"), subMenuSetting, json);
				if(i){
					$("ul.tree", ".dialog").jTree();
				}
...
function thissubmit(thiss){
	...
	$(".unit table tbody tr").each(function(){
		unit_code = $(this).find(".unit_code").val();
		unit_rate = $(this).find(".unit_rate").val();
		unit_base = $(this).find(".unit_base").val();
		if(unit_code && unit_rate ){
			unit_str += unit_code+'##'+unit_rate+'##'+unit_base+"||";
		}else{
	...
	if(!unit_str){
		alert('<?=  Yii::$service->page->translate->__('You add at least one unit') ?>');
		return false;
	}
	jQuery(".unit_info").val(unit_str);
	...
	jQuery(".cert_tree div.ckbox.checked").each(function(){
		cate_id = jQuery(this).find("input").val();
		cert_str += cate_id+",";
	});
	
	jQuery(".inputcategory").val(cate_str);
	jQuery(".inputcert").val(cert_str);
		...
		<div class="tabs" >
			<div class="tabsHeader">
				<div class="tabsHeaderContent">
					<ul>
						...
						<li><a href="javascript:;"><span><?=  Yii::$service->page->translate->__('Cert Info') ?></span></a></li>
						<li><a href="javascript:;"><span><?=  Yii::$service->page->translate->__('Unit Info') ?></span></a></li>
			...
			<div class="tabsContent" style="height:550px;overflow:auto;">
				...
				<div>
                    ...
                    <input type="hidden" value="" name="cert_file"  class="inputcert"/>
                    <ul class="cert_tree tree treeFolder treeCheck expand" >
                    </ul>
				</div>
				<div>
                      <div class="edit_p">
						<label><?=  Yii::$service->page->translate->__('Unit Info') ?>：</label>
						<input type="hidden" name="editFormData[unit]" class="unit_info"  />
						<div class="unit" style="float:left;width:700px;">
							<table style="">
								<thead>
									<tr>
										<th><?=  Yii::$service->page->translate->__('Unit Code') ?></th>
										<th><?=  Yii::$service->page->translate->__('Unit Rate') ?></th>
										<th><?=  Yii::$service->page->translate->__('Unit Base') ?></th>
										<th><?=  Yii::$service->page->translate->__('Action') ?></th>
									</tr>
								</thead>
								<tbody>
									<?php if(is_array($unit) && !empty($unit)){  ?>
										<?php foreach($unit as $one){ ?>
										<tr>
											<td>
												<input class="unit_code" type="text" value="<?= $one['unit_code'] ?>">
											</td>
											<td>
												<input class="unit_rate" type="text" value="<?= $one['unit_rate'] ?>">
											</td>
											<td>
												<input class="unit_base" type="text" value="<?= $one['unit_base'] ?>">
											</td>
											<td>
								...
								<tfoot style="text-align:right;">
									<tr>
										<td colspan="100" style="text-align:right;">						
											<a rel="2" style="text-align:right;" href="javascript:void(0)" class="addProductUnitInfo button">
												<span><?=  Yii::$service->page->translate->__('Add Unit') ?></span>
											</a>					
										</td>				
									</tr>			
								</tfoot>
							...
							<script>
							function unit_del(thiss){
                                        $(thiss).parent().parent().remove();
                                    }
								$(document).ready(function(){
									$(".addProductUnitInfo").click(function(){
										str = "<tr>";
										str +="<td><input class=\"unit_code textInput \" type=\"text\"   /> </td>";
										str +="<td><input class=\"unit_rate textInput\" type=\"text\"   /></td>";
										str +="<td><input class=\"unit_base textInput\" type=\"text\"   /></td>";
										str +="<td><i class='fa fa-trash-o' onclick=\"unit_del(this)\"></i></td>";
										str +="</tr>";
										$(".unit table tbody").append(str);
									});
