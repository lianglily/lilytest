...
namespace appadmin\local\local_modules\Catalog\block\productinfo;
...
class Manageredit extends AppadminbaseBlockEdit implements AppadminbaseBlockEditInterface
{
     ...
     */
    protected $_attrBlockName = '\appadmin\local\local_modules\Catalog\block\productinfo\index\Attr';
    ...
    public function getLastData()
    {
        ...
        return [
            ...
            'unit'         		=> isset($this->_one['unit'])?unserialize($this->_one['unit']):"",
	...
	public function getUnitInfo()
    {
        $this->_lang_attr = '';
        $this->_textareas = '';
        $editArr = $this->getUnitInfoAttr();
        $editBar = $this->getEditBar($editArr);
		
        return $this->_lang_attr.$editBar.$this->_textareas;
    }
	
	public function getUnitInfoAttr()
    {
        return [
            [
    ...
    protected function initParamType()
    {
        ...
        if ($unit) {
            ...
            if (is_array($arr) && !empty($arr)) {
                foreach ($arr as $ar) {
                    list($unit_code, $unit_rate,$unit_base) = explode('##', $ar);
           ...
           $this->_param['unit'] = serialize($unit_arr); 
        }
        
        //$this->_param['unit'] = $unit_arr;
