...
class Index extends AppadminbaseBlock implements AppadminbaseBlockInterface
{
    ...
    public function init()
    {
         ...
         * edit data url
         */
        $this->_editUrl = CUrl::getUrl('customer/newslettermanageredit/index');
        /*
         * delete data url
         */
        $this->_deleteUrl = CUrl::getUrl('customer/newslettermanageredit/index');
		
		$this->csvUrl=CUrl::getUrl('customer/newslettermanageredit/export');
        /*
    ...
    public function getTableTbodyHtml($data)
    {
        ...
        foreach ($data as $one) {
			...
			if($one['status']==1){
				$str .= '<td>
							<a title="' . Yii::$service->page->translate->__('Deactivate') . '" target="ajaxTodo" href="'.$this->_editUrl.'?'.$this->_primaryKey.'='.$one[$this->_primaryKey].'&status=10" class="btnDel"  csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '"  ><i class="fa fa-pencil"></i></a>
						</td>';
			}else{
				$str .= '<td>
							<a title="' . Yii::$service->page->translate->__('Activate') . '" target="ajaxTodo" href="'.$this->_editUrl.'?'.$this->_primaryKey.'='.$one[$this->_primaryKey].'&status=1" class="btnDel"  csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '"  ><i class="fa fa-pencil"></i></a>
						</td>';
			}
            $str .= '</tr>';
        }
		...
		<script>
                    $(document).ready(function(){
						$(".exportOrderExcel").click(function(){
                            var selectOrderIds = \'\';
                            $(\'.grid input:checkbox[name=ids]:checked\').each(function(k){
                                if(k == 0){
                            ...
                            if (!selectOrderIds) {
                                ...
                                url = "'.$this->csvUrl.'" ;
                                doPost(url, {"ids": selectOrderIds, "'.CRequest::getCsrfName().'": "'.CRequest::getCsrfValue() .'"});
                            }
                        });
					})</script>';
    }
