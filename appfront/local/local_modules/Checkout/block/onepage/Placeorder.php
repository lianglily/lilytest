            ...
            if ($this->checkOrderInfoAndInit($post)) {
                ...
                if ($gus_status && $save_address_status) {
                    ...
                    try {
                        ...
                        # 生成订单，扣除库存，但是，不清空购物车。
						if (!isset($post['billing']['shipping_date']) || empty($post['billing']['shipping_date'])) {
							Yii::$service->helper->errors->add('billing  shippng date can not empty');

							return false;
						}
						$this->_billing['shipping_date']=strtotime($post['billing']['shipping_date']);
