...
namespace appfront\local\local_modules\Checkout\block\onepage;
...
class Index
{
    ...
    public function initAddress()
    {
        ...
        $this->_address_list = Yii::$service->addresslocal->currentAddressList();
    ...
    public function getShippings($custom_shipping_method = '')
    {
		...
		$total=$cartProductInfo['base_product_total'];
		//var_dump($custom_shipping_method);
        // 当前的货运方式
        //$current_shipping_method = Yii::$service->shipping->getCurrentShippingMethod($custom_shipping_method, $cartShippingMethod, $country, $region, $product_final_weight,$total,$city);
        //var_dump($current_shipping_method);
        
		//$this->_shipping_method = $current_shipping_method;
        // 得到所有，有效的shipping method
        $shippingArr = $this->getShippingArr($product_final_weight, $custom_shipping_method, $country, $region,$total,$city);
     ...
     */
    public function getShippingArr($weight, $current_shipping_method, $country, $region="*",$total=0,$city="*")
    {
        $available_shipping = Yii::$service->shipping->getAvailableShippingMethods($country, $region, $weight,$total,$city);
        ...
        if (is_array($available_shipping) && !empty($available_shipping)) {
            foreach ($available_shipping as $method=>$shipping) {
                ...
                $name = $shipping['name'];
                if($shipping['formula']=='csv'){
	                // 得到运费的金额
	                $cost = Yii::$service->shipping->getShippingCost($method, $shipping, $weight, $country, $region,$total,$city);
	                ...
	                $arr[] = [
	                    ...
	                    'defaultCountry' => ["country"=>$country,"region"=>$region,"city"=>$city]
	                ];
                }else{
	                ...
	                //var_dump($shipping);
	                $currentCurrencyCost = $cost['currCost'];
	                $symbol = Yii::$service->page->currency->getCurrencyInfo()['code'];
	                
	                
	                if (Yii::$service->shipping->isCountryLimits1($shipping['country'],$country,$region,$city)) {
	                	$this->_shipping_method = $method;
	                    $checked = true;
	                } else {
	                    $checked = '';
    ...
    public function ajaxUpdateOrderAndShipping()
    {
		...
		$city = \Yii::$service->helper->htmlEncode($city);
        if ($address_id) {
            $this->_address_id = $address_id;
            $addressModel = Yii::$service->customer->address->getByPrimaryKey($this->_address_id);
        ...
        if ($this->_country && $this->_state) {
            ...
            // 计算运费。
            $avaiable_method = Yii::$service->shipping->getAvailableShippingMethods($country,$region,$product_weight,$total,$city);
            //var_dump($shipping_method);
            $shippingInfo = $avaiable_method[$this->_shipping_method];
            $shippingCost = Yii::$service->shipping->getShippingCost($shipping_method, $shippingInfo, $product_weight, $country, $state,$city);
