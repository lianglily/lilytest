...
namespace appfront\local\local_modules\Customer\block\address;
...
class Edit
{
    ...
    public function initAddress()
    {
        ...
        */
        ...
        $countryid=is_numeric($this->_address['country'])?$this->_address['country']-1:-1;

        if(isset($areas[$countryid])){
        	
        	if($areas[$countryid]['translate']){
        		$this->_address['countryvalue'] = Yii::$service->page->translate->__($areas[$countryid]['translate']);
        	}else{
        		$this->_address['countryvalue'] = $areas[$countryid]['name'];
        	}
        	$stateid=is_numeric($this->_address['state'])?$this->_address['state']-1:-1;
        	
        	if(isset($areas[$countryid]['cityList'][$stateid])){
        		if($areas[$countryid]['cityList'][$stateid]['translate']){
            		$this->_address['statevalue'] = Yii::$service->page->translate->__($areas[$countryid]['cityList'][$stateid]['translate']);
            	}else{
            		$this->_address['statevalue'] = $areas[$countryid]['cityList'][$stateid]['name'];
            	}
        	}
        	$cityid=is_numeric($this->_address['city'])?$this->_address['city']-1:-1;
        	if(isset($areas[$countryid]['cityList'][$stateid]['areaList'][$cityid])){
        		if($areas[$countryid]['cityList'][$stateid]['areaList'][$cityid]['translate']){
            		$this->_address['cityvalue'] = Yii::$service->page->translate->__($areas[$countryid]['cityList'][$stateid]['areaList'][$cityid]['translate']);
            	}else{
            		$this->_address['cityvalue'] = $areas[$countryid]['cityList'][$stateid]['areaList'][$cityid]['name'];
            	}
            	$weekday=$areas[$countryid]['cityList'][$stateid]['areaList'][$cityid]['weekday'];
        	}
        }else{
        
        	$this->_address['countryvalue'] = Yii::$service->helper->country->getCountryNameByKey($this->_address['country']);
        	$this->_address['statevalue'] = Yii::$service->helper->country->getStateByContryCode($this->_address['country'],$this->_address['state']);
			$this->_address['cityvalue'] = $this->_address['city'];
        }
