...
<div class="onestepcheckout-summary">
	<table class="onestepcheckout-summary">
		<thead>
			<tr>
				<th class="image"></th>
				<th class="name"><?= Yii::$service->page->translate->__('Product Name') ?></th>
			...
			<tr>
				<td class='image'>
					<a href="<?= $product['url'] ?>" title="<?= $product['name'] ?>" class="product-image">
						<img alt="<?= $product['name'] ?>" src="<?= Yii::$service->product->image->getResize($product['image'],[100,100],false) ?>" alt="2121" width="75" height="75">
					</a>
				...
				<td class="total"><span class="price"><?=  $currency_info['code'];  ?><?= Format::price($product['product_row_price']); ?></span></td>
			</tr>
			<?php endforeach; ?>			
		</tbody>
	</table>
	...
	<table class="onestepcheckout-totals">
		<tbody>
			<tr>
				...
				<td class="value">
					<span class="price"><?=  $currency_info['code'];  ?><?= Format::price($cart_info['product_total']); ?></span>       
				</td>
			</tr>
			<tr>
				<td class="title"><?= Yii::$service->page->translate->__('Shipping Cost') ?></td>
				<td class="value">
					<span class="price"><?=  $currency_info['code'];  ?><?= Format::price($cart_info['shipping_cost']); ?></span> 
				</td>
			</tr>
			<tr>
				<td class="title"><?= Yii::$service->page->translate->__('Discount') ?></td>
				<td class="value">
					<span class="price">-<?=  $currency_info['code'];  ?><?= Format::price($cart_info['coupon_cost']); ?></span> 
				</td>
			</tr>
			<tr class="grand-total">
				<td class="title"><?= Yii::$service->page->translate->__('Grand Total') ?></td>
				<td class="value">
					<span class="price"><?=  $currency_info['code'];  ?><?= Format::price($cart_info['grand_total']) ?></span>   
				</td>
			</tr>						
        </tbody>
	</table>
