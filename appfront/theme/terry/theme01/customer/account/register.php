				...
				<ul class="form-list">
					<li class="fields">
						<div class="customer-name">
							<div class="field name-companycn">
								<label for="companycn" class="required"><em>*</em><?= Yii::$service->page->translate->__('Company CN'); ?></label>
								<div class="input-box">
									<input id="companycn" name="editForm[companycn]" value="<?= $companycn ?>" title="Company CN Name" maxlength="40" class="input-text required-entry" type="text">
								</div>
							</div>
							<div class="field name-companyen">
								<label for="companyen" class="required"><em>*</em><?= Yii::$service->page->translate->__('Company EN'); ?></label>
								<div class="input-box">
									<input id="companyen" name="editForm[companyen]" value="<?= $companyen ?>" title="Company EN Name" maxlength="40" class="input-text required-entry" type="text">
								</div>
							</div>
					...
					<li class="fields">
						<div class="customer-name">
							...
							<div class="field name-firstname">
								<label for="contact_to" class="required"><em>*</em><?= Yii::$service->page->translate->__('Tel'); ?></label>
								<div class="input-box">
									<input name="editForm[tel]" id="tel" value="<?= $tel ?>" title="Tel" class="input-text required-entry" type="text">
								</div>
							</div>
							<div class="clear"></div>
						</div>
					...
					<li class="fields">
						<div class="customer-name">
							<div class="field name-firstname">
								<label for="contact_to" class="required"><em>*</em><?= Yii::$service->page->translate->__('Office Tel'); ?></label>
								<div class="input-box">
									<input name="editForm[contact]" id="contact_to" value="<?= $contact ?>" title="Contact To" class="input-text required-entry" type="text">
								</div>
							</div>
							<div class="field name-firstname">
								<label for="contact_to" ><?= Yii::$service->page->translate->__('Fax'); ?></label>
								<div class="input-box">
									<input name="editForm[fax]" id="fax" value="<?= $fax ?>" title="Fax" class="input-text " type="text">
								</div>
							</div>
							<div class="clear"></div>
						</div>
					...
					<li class="fields">
						...
						<div class="field">
                            ...
                            <div class="input-box register-captcha">
								...
								<img class="login-captcha-img" alt="click refresh" title="click refresh" src="<?= Yii::$service->url->getUrl('site/helper/captcha'); ?>?<?php echo md5(time() . mt_rand(1,10000));?>" align="absbottom" onclick="this.src='<?= Yii::$service->url->getUrl('site/helper/captcha'); ?>?'+Math.random();"></img>
	...
	$("#js_registBtn").click(function(){
		...
		$(".account-register .required-entry").each(function(){
			...
			if(!val){
				...
				$(this).parent().append('<div class="validation-advice" id="advice-required-entry-firstname" ><?= $requiredValidate; ?></div>');
				validate = 0;
			}
		});
		...
		$(".account-register .validate-email").each(function(){
			...
			if(email){
				if(!$(this).hasClass("validation-failed")){
					if(!myreg.test(email)){
						$(this).parent().append('<div class="validation-advice" id="advice-validate-email-email_address" ><?= $emailFormatValidate; ?></div>');
						$(this).addClass("validation-failed");
						validate = 0;
					}
				}
		...
		if(firstNameLength < minNameLength || firstNameLength > maxNameLength){
			if(!$("#firstname").hasClass("validation-failed")){
				//alert(111);
				$("#firstname").parent().append('<div class="validation-advice" id="min_lenght" ><?= $firstNameLenghtValidate; ?> '+minNameLength+' , '+maxNameLength+'</div>');
		...
		if(lastNameLength < minNameLength || lastNameLength > maxNameLength){
			if(!$("#lastname").hasClass("validation-failed")){
				//alert(111);
				$("#lastname").parent().append('<div class="validation-advice" id="min_lenght" ><?= $lastNameLenghtValidate; ?> '+minNameLength+' , '+maxNameLength+'</div>');
		...
		if(passwordLength < minPassLength || passwordLength > maxPassLength){
			if(!$("#password").hasClass("validation-failed")){
				//alert(111);
				$("#password").parent().append('<div class="validation-advice" id="min_lenght" ><?= $passwordLenghtValidate; ?> </div>');
		...
		if(confirmation != password){
			if(!$("#confirmation").hasClass("validation-failed")){
				//alert(111);
				$("#confirmation").parent().append('<div class="validation-advice" id="min_lenght" ><?= $passwordMatchValidate; ?></div>');
