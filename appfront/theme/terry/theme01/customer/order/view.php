					...
					<div class="col2-set order-info-box">
						<div class="col-1">
							<div class="box">
                                <div class="box-content">
                                    <address>
                                    	<label class="label_info"><?= Yii::$service->page->translate->__('Order Date');?>：</label><label  class="label_info_left"><?=  date('Y-m-d',$created_at); ?></label><br>
                                		<label class="label_info"><?= Yii::$service->page->translate->__('Shipping Date');?>：</label><label   class="label_info_left"><?=  date('Y-m-d',$shipping_date); ?></label><br>
                                		<label class="label_info"><?= Yii::$service->page->translate->__('Order Total');?>：</label><label   class="label_info_left"><?= $order_currency_code ?><?=  Format::price($grand_total); ?></label><br>
                                		<label class="label_info"><?= Yii::$service->page->translate->__('Payment Method');?>：</label><label   class="label_info_left"><?=  Yii::$service->page->translate->__($paymentConfig['label']) ?></label><br>
                                		<label class="label_info"><?= Yii::$service->page->translate->__('Payment Status');?>：</label><label   class="label_info_left"><?= Yii::$service->order->getSelectStatusArr()[$order_status] ;?></label><br>
                                		<label class="label_info"><?= Yii::$service->page->translate->__('Order Status');?>：</label><label   class="label_info_left"><?= Yii::$service->page->translate->__(Yii::$service->order->getSelectProcessStatusArr()[$order_process_status]) ;?></label><br>
                                    </address>
                                </div>
                            </div>				
                        </div>
						<div class="col-2">
							<div class="box">
                                ...
                                <div class="box-content">
                                	<div class="box-title">
	                                    <?= Yii::$service->page->translate->__('Shipping Address');?>:
	                                </div>
                                    ...
                                    <div class="layui-upload">
                                      <div class="layui-upload-list display_inline-table" >
                                      	<?php if($credentials_payment){?>
									    <img alt="credentialsupload" class="layui-upload-img width_100_height_100" id="demo1"  src="<?=  Yii::$service->url->getUrl('checkout/credentialsupload/imageshow',['msg' => $credentials_payment]);?>">
									    <button type="button" class="layui-btn" id="credentials_payment"><?= Yii::$service->page->translate->__('Replace Credentials');?></button>
										<?php }else{ ?>
									   <img alt="credentialsupload" class="layui-upload-img width_100_height_100" id="demo1" style="display:none">
									    <button type="button" class="layui-btn" id="credentials_payment"><?= Yii::$service->page->translate->__('Upload Credentials');?></button>
										<?php } ?>
									    <p id="demoText"></p>
									  </div>
									</div> 
							...
							<tbody class="odd">
								<?php if(is_array($products) && !empty($products)):  ?>
									...
									<!-- tr id="order-item-row" class="border first">	
										...
										<td>
											<a href="<?=  Yii::$service->url->getUrl($product['redirect_url']) ; ?>">
												<img alt="<?= $product['name'] ?>" src="<?= Yii::$service->product->image->getResize($product['image'],[100,100],false) ?>" alt="<?= $product['name'] ?>" width="75" height="75">
											</a>
										</td>
										<td><?= $product['sku'] ?></td>
										<td class="a-right">
											<span class="price-excl-tax">
												<span class="cart-price">
													<span class="price"><?= $order_currency_code ?><?= Format::price($product['price']); ?></span>                    
												</span>
											</span>
											<br>
										</td>
...
$(document).ready(function(){
	layui.use('layer', function(){
		...
		uploadInst=upload.render({
		  elem: '#credentials_payment'
		  ,url: "<?=  Yii::$service->url->getUrl('checkout/credentialsupload/imageupload',['order_id' => $increment_id]);?>"
		  ,accept:'images'
		  ,acceptMime:'image/*'
		  ,size:500
		  ,drag:true
		  ...
		  ,before: function(obj){
		      ...
		      obj.preview(function(index, file, result){
				$('#demo1').css('display', 'inline'); //图片链接（base64）
		    ...
		    ,done: function(res){
		      //如果上传失败
		      if(res.code > 0){
		        return layer.msg('<?= Yii::$service->page->translate->__('Upload Fail');?>');
		      }
		      //上传成功
		    }
		    ,error: function(){
		      ...
		      demoText.html('<span style="color: #FF5722;"><?= Yii::$service->page->translate->__('Upload Fail');?></span> <a class="layui-btn layui-btn-xs demo-reload"><?= Yii::$service->page->translate->__('Retry');?></a>');
