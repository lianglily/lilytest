...
<?php  if(is_array($tier_price) && !empty($tier_price) && isset($tier_price[0]['qty']) ): ?>
<div class="label"> <?= Yii::$service->page->translate->__('Wholesale Prices :'); ?></div>
	<table >
		...
		<tr>
			<td></td>
		<?php  foreach($tier_price as $one):  ?>
			<td><button type="button" class="layui-btn layui-btn-sm btn_qty" data-qty="<?= $one['qty']?>">
				<?= Yii::$service->page->translate->__('Add'); ?>(<?= $one['qty']?>)
				</button>
			</td>
		<?php  endforeach;  ?>
		</tr>
