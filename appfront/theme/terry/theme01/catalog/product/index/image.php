...
//echo "{$small_img_width}-{$small_img_height}-{$middle_img_width}";
?>
<?php  $main_img = isset($image['main']['image']) ? $image['main']['image'] : '' ?>
<div class="product-main-img" style='width:<?= $middle_img_width ?>px'>
	<img alt="<?= $main_img ?>" style="width:<?= $middle_img_width ?>px;max-height:450px" id="zoom_03" src="<?= Yii::$service->product->image->getResize($main_img,$middle_img_width,false) ?>" data-zoom-image="<?= Yii::$service->product->image->getUrl($main_img);  ?>"/>
</div>
<?php
...
?>
		...
		?>
						<a href="#" data-image="<?= Yii::$service->product->image->getResize($image,$middle_img_width,false) ?>" data-zoom-image="<?= Yii::$service->product->image->getUrl($image);  ?>">
							<img alt="<?= $image ?>" class="elevateZoom lazyOwl" id="img_01" src="<?= Yii::$service->product->image->getResize($image,[$small_img_width,$small_img_height],false) ?>" />
						</a>
		<?php
			endforeach;
		?>
