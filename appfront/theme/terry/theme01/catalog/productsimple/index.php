		...
		<div class="product_page">
			<div class="product_view">
				...
				<div class="product_info" id="product_page_info">
					<h1 class="product_info_h1"><?= $name; ?></h1>
					<div>
						<div class="rbc_cold">
							<span>
								<span class="average_rating"><?= Yii::$service->page->translate->__('Average rating'); ?> :</span>
								<span class="review_star review_star_<?= round($reviw_rate_star_average) ?> text_bold"  itemprop="average"></span>  
					...
					<div class="product_info_section">
						<div class="product_options">
							<?= Yii::$service->page->widget->render('product/options', ['options' => $options,'mode'=>false]); ?>
						...
						<?php }?>
							...
							<div class="myFavorite_nohove button" id="myFavorite">
								
								<a href="javascript:" url="<?= Yii::$service->url->getUrl('catalog/favoriteproduct/add'); ?>"   class="addheart" id="divMyFavorite" rel="nofollow" >
								<span class="layui-icon layui-icon-heart-fill" ></span>  	<?= Yii::$service->page->translate->__('Add to Favorites'); ?>
								</a>				
							</div>
							<div class="clear"></div>
						</div>
			...
			<div>
				<?= Yii::$service->page->widget->render('product/buy_also_buy', ['products' => $buy_also_buy,'mode'=>false]); ?>
			</div>
				...
				<div id="text">  
                        ...
                        <div class="img-section text_center" >
                            <?php   if(is_array($image_detail)):  ?>
                                <?php foreach($image_detail as $image_detail_one): ?>
                                    <br/>
                                    <img alt="<?= $image_detail_one['image'] ?>" class="js_lazy" src="<?= Yii::$service->image->getImgUrl('images/lazyload.gif');   ?>" data-original="<?= Yii::$service->product->image->getUrl($image_detail_one['image']);  ?>"  />
					...
					<div class="text-reviews" id="text-reviews" >
						<?php # review部分。
                            ...
                            $reviw_rate_star_info['mode']=false;
				...
				jQuery.ajax({
					...
					success:function(data, textStatus){ 
						if(data.status == 'success'){
							...
							$("#js_cart_items",window.parent.document).html(items_count);
							$(".addProductToCart").removeClass("dataUp");
							layer.msg("<?= Yii::$service->page->translate->__('Add to cart success!'); ?>");
							setTimeout(function(){var index = parent.layer.getFrameIndex(window.name);
			...
			jQuery.ajax({
				...
				success:function(data, textStatus){ 
					...
					if(data.status == 'success'){
						...
						if(param["status"]){
							count=$("#js_favour_num",window.parent.document).text();
							$('#divMyFavorite').addClass('act');
							$(this).addClass('act');
							$("#js_favour_num",window.parent.document).text(parseInt(count)+1);
